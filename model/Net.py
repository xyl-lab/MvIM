...
from .Layer import MLP, ProjectedAttentionLayer, EmbeddedAttentionLayer, BackboneSAITS
...
class INet(nn.Module):
    ...
    def __init__(
        ...
        spatio_attention_mask: bool = False,
    ):
        super().__init__()
        ...
        self.spatio_attention_mask = spatio_attention_mask
        self.learnable_embedding = nn.init.xavier_uniform_(
            nn.Parameter(
                torch.empty(self.in_steps, self.n_nodes,
                            self.learnable_embedding_dim)
            )
        )

        self.readout = MLP(self.model_dim, self.model_dim,
                           output_dim, n_layers=2)
    ...
    def forward(self, inputs: dict, training: bool = True, temporal_attention_mask: bool = False, spatio_attention_mask: bool = False) -> dict:
        x, missing_mask = inputs["X"], inputs["missing_mask"]
        # determine the attention mask
        if (training and self.temporal_attention_mask) or (
            (not training) and temporal_attention_mask
        ):
            temporal_attention_mask = (
                1 - torch.eye(self.in_steps)).to(x.device)
            # then broadcast on the batch axis
            temporal_attention_mask = temporal_attention_mask.unsqueeze(0)
        else:
            temporal_attention_mask = None
        if (training and self.spatio_attention_mask) or (
            (not training) and spatio_attention_mask
        ):
            spatio_attention_mask = (
                1 - torch.eye(self.n_nodes)).to(x.device)
            # then broadcast on the batch axis
            spatio_attention_mask = spatio_attention_mask.unsqueeze(0)
        else:
            spatio_attention_mask = None
        ...
        for att_t, att_s in zip(self.attn_layers_t, self.attn_layers_s):
            x = att_t(x, temporal_attention_mask)
            x = att_s(x, self.learnable_embedding,
                      spatio_attention_mask, dim=1)
        ...
        imputed_data = missing_mask * \
            inputs["X"] + (1 - missing_mask) * reconstruction
